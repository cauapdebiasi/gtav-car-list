...
export async function getStaticPaths() {
  ...
  const res = await fetch("https://gta.now.sh/api/vehicles/classes");
  const classes = await res.json();

  // Get the paths we want to pre-render based on posts
  const paths = Object.keys(classes).map((className) => ({
    params: { className },
  }));
...
export async function getStaticProps({ params }) {
  ...
  const res = await fetch(
    `https://gta.now.sh/api/vehicles/class/${params.className}`
  );
  const vehicleList = await res.json();

  // Pass post data to the page via props
  return { props: { vehicleList, className: params.className } };
}

export default function ClassName({ vehicleList, className }) {
  ...
  return (
    <Layout>
      <h2 className="uppercase my-2 text-[24px]">{className}</h2>
